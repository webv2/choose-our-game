document.addEventListener('DOMContentLoaded', () => {
    ...
    const orderingFilter = document.getElementById('ordering-filter'); // New ordering filter element
    ...
    const saveState = () => {
        localStorage.setItem('chooseOurGameState', JSON.stringify({
            ...
            filters: {
                ...
                ordering: orderingFilter.value, // Save ordering
    ...
    const getDatesForFilter = (filterValue) => {
        ...
        switch (filterValue) {
            ...
            case 'next-month':
                return `${formatDate(new Date(y, m + 1, 1))},${formatDate(new Date(y, m + 2, 0))}`;
            case 'popular-week':
    ...
    const fetchGames = async (page = 1, append = false) => {
        ...
        const ordering = orderingFilter.value; // Get the ordering value
