document.addEventListener('DOMContentLoaded', () => {
    ...
    // --- State & Persistence ---
    const saveState = () => { localStorage.setItem('chooseOurGameState', JSON.stringify({ theme: document.body.classList.contains('dark-theme') ? 'dark' : 'light', filters: { search: searchBar.value, dynamic: dynamicFilter.value, genre: genreFilter.value, tags: tagsFilter.value, library: libraryToggleBtn.dataset.toggled === 'true', filtersVisible: !filtersPanel.classList.contains('hidden'), years: Array.from(selectedYears), ordering: orderingFilter.value } })); };
    const loadState = () => { const stateString = localStorage.getItem('chooseOurGameState'); if (!stateString) return; const state = JSON.parse(stateString); document.body.className = state.theme === 'dark' ? 'dark-theme' : 'light-theme'; searchBar.value = state.filters.search || ''; dynamicFilter.value = state.filters.dynamic || ''; genreFilter.value = state.filters.genre || ''; tagsFilter.value = state.filters.tags || ''; orderingFilter.value = state.filters.ordering || ''; if(state.filters.years) { selectedYears = new Set(state.filters.years); } if (state.filters.library) { libraryToggleBtn.dataset.toggled = 'true'; gamesContainer.classList.add('filter-library'); } if (state.filters.filtersVisible) { filtersPanel.classList.remove('hidden'); toggleFiltersBtn.textContent = 'Hide Filters'; } };
...
// --- Data Fetching & Display ---
    ...
    loadState();
    populateDateRanges();
    fetchGenres(); // This master function now handles the initial load and filter application
    ...
    [genreFilter, tagsFilter, dynamicFilter, orderingFilter].forEach(filter => filter.addEventListener('change', () => { if (filter === dynamicFilter && filter.value) { selectedYears.clear(); orderingFilter.value = ''; } if (filter === orderingFilter && filter.value) { dynamicFilter.value = ''; } updateDateFilterButtonText(); applyFilters();}));
