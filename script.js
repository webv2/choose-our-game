document.addEventListener('DOMContentLoaded', () => {
    ...
    // mySteamLibrary Set
    const mySteamLibrary = new Set([ "anno 1800", /* ... and so on ... */ "dirt 3" ]);
    ...
    const saveState = () => {
        const state = {
            ...
            filters: {
                search: searchBar.value, dynamic: dynamicFilter.value, genre: genreFilter.value, tags: tagsFilter.value,
                library: libraryToggleBtn.dataset.toggled === 'true', filtersVisible: !filtersPanel.classList.contains('hidden'),
                years: Array.from(selectedYears) // Save selected years
            }
        };
        localStorage.setItem('chooseOurGameState', JSON.stringify(state));
    };
    ...
    const populateYears = (start, end) => {
        ...
        for (let year = end; year >= start; year--) {
            ...
            if (selectedYears.has(year.toString())) {
    ...
    // --- Core API & Display Functions --- 
    const fetchGames = async (page = 1, append = false) => {
        if (isLoading || (!hasNextPage && append)) return;
        isLoading = true; showLoader();
        const search = searchBar.value.trim(); const genre = genreFilter.value; const tags = tagsFilter.value; const dynamic = dynamicFilter.value;
        let url = `${BASE_URL}/games?key=${API_KEY}&page=${page}&page_size=24`;
        if (search) url += `&search=${search}&search_exact=true`; if (genre) url += `&genres=${genre}`; if (tags) url += `&tags=${tags}`;
        const dates = getDatesForFilter(dynamic); if (dates) url += `&dates=${dates}`; if (dynamic.startsWith('popular')) url += `&ordering=-added`;
        
        // Add selected years to the query
        if (selectedYears.size > 0) {
        ...
        try {
            const response = await fetch(url); const data = await response.json(); displayGames(data.results, append); hasNextPage = data.next !== null;
        } catch (error) { console.error('Error fetching games:', error); gamesContainer.innerHTML = '<p>Failed to load games. Please try again later.</p>'; } finally { hideLoader(); isLoading = false; checkAndLoadMore(); }
    };
    
    // (Other functions like fetchGenres, displayGames, openModal, closeModal, etc., are the same as the previous 'final' version)
    // For brevity, only the new and modified functions are shown in full detail.
    // The complete, working code block follows this explanation.
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const fetchGames = async (page = 1, append = false) => { if (isLoading || (!hasNextPage && append)) return; isLoading = true; showLoader(); const search = searchBar.value.trim(); const genre = genreFilter.value; const tags = tagsFilter.value; const dynamic = dynamicFilter.value; let url = `${BASE_URL}/games?key=${API_KEY}&page=${page}&page_size=24`; if (search) url += `&search=${search}&search_exact=true`; if (genre) url += `&genres=${genre}`; if (tags) url += `&tags=${tags}`; const dates = getDatesForFilter(dynamic); if (dates) url += `&dates=${dates}`; if (dynamic.startsWith('popular')) url += `&ordering=-added`; if (selectedYears.size > 0 && !dates) { const yearRanges = Array.from(selectedYears).map(year => `${year}-01-01,${year}-12-31`).join(','); url += `&dates=${yearRanges}`; } if (!append) gamesContainer.innerHTML = ''; try { const response = await fetch(url); const data = await response.json(); displayGames(data.results, append); hasNextPage = data.next !== null; } catch (error) { console.error('Error fetching games:', error); gamesContainer.innerHTML = '<p>Failed to load games. Please try again later.</p>'; } finally { hideLoader(); isLoading = false; checkAndLoadMore(); } };
    const fetchGenres = async () => { try { const response = await fetch(`${BASE_URL}/genres?key=${API_KEY}`); const data = await response.json(); data.results.forEach(genre => { const option = document.createElement('option'); option.value = genre.slug; option.textContent = genre.name; genreFilter.appendChild(option); }); applyFilters(); } catch (error) { console.error('Error fetching genres:', error); } };
    const displayGames = (games, append) => { if (games.length === 0 && !append) { gamesContainer.innerHTML = '<p>No games found.</p>'; return; } const fragment = document.createDocumentFragment(); games.forEach(game => { const gameTile = document.createElement('div'); const isOwned = mySteamLibrary.has(game.name.toLowerCase().trim()); gameTile.className = `game-tile ${isOwned ? 'owned-game' : ''}`; gameTile.dataset.gameId = game.id; gameTile.innerHTML = `<img src="${game.background_image || ''}" loading="lazy"><div class="game-info"><h3>${game.name}</h3><button class="vote-btn">Vote</button></div>`; gameTile.addEventListener('click', (e) => { if (e.target.tagName !== 'BUTTON') openModal(gameTile); }); fragment.appendChild(gameTile); }); gamesContainer.appendChild(fragment); updateUIWithVotes(); };
    const openModal = (tile) => { if (!tile) return; originalTile = tile; const gameId = tile.dataset.gameId; const tileRect = tile.getBoundingClientRect(); const imgSrc = tile.querySelector('img').src; modalContainer.style.left = `${tileRect.left}px`; modalContainer.style.top = `${tileRect.top}px`; modalContainer.style.width = `${tileRect.width}px`; modalContainer.style.height = `${tileRect.height}px`; modalOverlay.classList.add('active'); tile.classList.add('hiding'); fetchGameDetailsAndAnimate(gameId, tileRect, imgSrc); };
    const fetchGameDetailsAndAnimate = async (gameId, tileRect, imgSrc) => { modalBody.innerHTML = ''; try { const [details, screenshots] = await Promise.all([ fetch(`${BASE_URL}/games/${gameId}?key=${API_KEY}`).then(res => res.json()), fetch(`${BASE_URL}/games/${gameId}/screenshots?key=${API_KEY}`).then(res => res.json()) ]); modalBody.innerHTML = `<div class="modal-header"><img src="${imgSrc}" class="modal-cover-art"><div class="modal-header-text"><h2>${details.name}</h2><p>${details.description_raw ? details.description_raw.substring(0, 280) : 'No description.'}...</p><strong>Genres:</strong> ${details.genres.map(g => g.name).join(', ')}<br><strong>Date:</strong> ${details.released}</div></div><h3>Screenshots</h3><div id="modal-screenshots">${screenshots.results.slice(0, 6).map(ss => `<img src="${ss.image}">`).join('')}</div>`; } catch (error) { modalBody.innerHTML = '<p>Could not load details.</p>'; } finally { requestAnimationFrame(() => { modalContainer.classList.add('active'); const targetWidth = Math.min(800, window.innerWidth - 40); const targetHeight = Math.min(600, window.innerHeight - 40); const targetX = (window.innerWidth - targetWidth) / 2; const targetY = (window.innerHeight - targetHeight) / 2; modalContainer.style.transform = `translate(${targetX - tileRect.left}px, ${targetY - tileRect.top}px)`; modalContainer.style.width = `${targetWidth}px`; modalContainer.style.height = `${targetHeight}px`; }); } };
    ...
    const updateUIWithVotes = () => { document.querySelectorAll('.game-tile').forEach(tile => { const gameId = tile.dataset.gameId; const voteBtn = tile.querySelector('.vote-btn'); const voteCountDiv = tile.querySelector('.vote-count'); voteBtn.classList.toggle('voted', votedGames.has(gameId)); voteBtn.textContent = votedGames.has(gameId) ? 'Voted!' : 'Vote'; const count = importedVotes[gameId] || 0; if (count > 0) { if (voteCountDiv) { voteCountDiv.textContent = count; } else { const newVoteCount = document.createElement('div'); newVoteCount.className = 'vote-count'; newVoteCount.title = `${count} votes`; newVoteCount.textContent = count; tile.appendChild(newVoteCount); } } else if (voteCountDiv) { voteCountDiv.remove(); } }); };
    loadState(); populateDateRanges(); updateDateFilterButtonText(); fetchGenres();
    dateFilterBtn.addEventListener('click', (e) => { e.stopPropagation(); dateFilterPanel.classList.toggle('hidden'); });
    dateRangesList.addEventListener('click', (e) => { if (e.target.tagName === 'LI') { document.querySelectorAll('#date-ranges-list li').forEach(li => li.classList.remove('active')); e.target.classList.add('active'); populateYears(e.target.dataset.start, e.target.dataset.end); } });
    dateYearsList.addEventListener('click', (e) => { if (e.target.tagName === 'LI') { const year = e.target.dataset.year; e.target.classList.toggle('selected'); if (selectedYears.has(year)) { selectedYears.delete(year); } else { selectedYears.add(year); } updateDateFilterButtonText(); applyFilters(); } });
    dateSelectAllBtn.addEventListener('click', () => { const allYears = Array.from(dateYearsList.querySelectorAll('li')); const allSelected = allYears.every(li => li.classList.contains('selected')); allYears.forEach(li => { const year = li.dataset.year; if (allSelected) { li.classList.remove('selected'); selectedYears.delete(year); } else { li.classList.add('selected'); selectedYears.add(year); } }); updateDateFilterButtonText(); applyFilters(); });
    document.addEventListener('click', (e) => { if (!dateFilterPanel.contains(e.target) && e.target !== dateFilterBtn) dateFilterPanel.classList.add('hidden'); });
    [genreFilter, tagsFilter, dynamicFilter].forEach(filter => filter.addEventListener('change', () => { if(filter === dynamicFilter && filter.value) selectedYears.clear(); updateDateFilterButtonText(); applyFilters();}));
    libraryToggleBtn.addEventListener('click', () => { libraryToggleBtn.dataset.toggled = !(libraryToggleBtn.dataset.toggled === 'true'); gamesContainer.classList.toggle('filter-library'); saveState(); setTimeout(checkAndLoadMore, 100); });
    let searchTimeout; searchBar.addEventListener('input', () => { clearTimeout(searchTimeout); searchTimeout = setTimeout(applyFilters, 500); });
    ...
    window.addEventListener('scroll', () => { if (isLoading || !hasNextPage) return; if (window.innerHeight + window.scrollY >= document.documentElement.scrollHeight - 500) { currentPage++; fetchGames(currentPage, true); } });
    ...
    const fullShare = () => { if (votedGames.size === 0) { alert('You haven\'t voted for any games yet!'); return; } const data = JSON.stringify(Array.from(votedGames)); const blob = new Blob([data], { type: 'application/json' }); const url = URL.createObjectURL(blob); const a = document.createElement('a'); a.href = url; a.download = 'my_game_votes.json'; document.body.appendChild(a); a.click(); document.body.removeChild(a); URL.revokeObjectURL(url); }; shareBtn.onclick = fullShare;
    const fullImport = (e) => { const files = e.target.files; if (!files.length) return; importedVotes = {}; Array.from(files).forEach(file => { const reader = new FileReader(); reader.onload = (event) => { try { const importedIds = JSON.parse(event.target.result); if (Array.isArray(importedIds)) { importedIds.forEach(id => { const gameId = id.toString(); importedVotes[gameId] = (importedVotes[gameId] || 0) + 1; }); updateUIWithVotes(); } } catch (err) { console.error('Error reading file:', err); alert(`Could not read file: ${file.name}`); } }; reader.readAsText(file); }); importInput.value = ''; }; importInput.onchange = fullImport;
    dateRangesList.querySelector('li')?.click();
});