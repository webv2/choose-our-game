document.addEventListener('DOMContentLoaded', () => {
    ...
    loadState(); populateDateRanges(); fetchGenres();
    dateFilterBtn.addEventListener('click', (e) => { e.stopPropagation(); dateFilterPanel.classList.toggle('hidden'); if(!dateRangesList.querySelector('.active')) dateRangesList.querySelector('li')?.click(); });
    ...
    dateYearsList.addEventListener('click', (e) => { if (e.target.tagName === 'LI') { const year = e.target.dataset.year; e.target.classList.toggle('selected'); if (selectedYears.has(year)) { selectedYears.delete(year); } else { selectedYears.add(year); } dynamicFilter.value = ''; updateDateFilterButtonText(); applyFilters(); } });
    dateSelectAllBtn.addEventListener('click', () => { const allYears = Array.from(dateYearsList.querySelectorAll('li')); const allSelected = allYears.every(li => li.classList.contains('selected')); allYears.forEach(li => { const year = li.dataset.year; if (allSelected) { li.classList.remove('selected'); selectedYears.delete(year); } else { li.classList.add('selected'); selectedYears.add(year); } }); dynamicFilter.value = ''; updateDateFilterButtonText(); applyFilters(); });
    ...
    const showLoader = () => loader.style.display = 'block'; const hideLoader = () => loader.style.display = 'none';
});